...
from fate_client.pipeline.components.fate.nn.torch.base import TorchModule, TorchOptimizer, Sequential
...
def get_config_of_default_runner(
    ...
    if model is not None and not isinstance(model, (TorchModule, Sequential, ModelLoader)):
        raise ValueError(f'The model is of type {type(model)}, not TorchModule, Sequential, or ModelLoader. Remember to use patched_torch_hook for passing NN Modules or Optimizers.')
    
    if optimizer is not None and not isinstance(optimizer, (TorchOptimizer, Loader)):
        raise ValueError(f'The optimizer is of type {type(optimizer)}, not TorchOptimizer or Loader. Remember to use patched_torch_hook for passing NN Modules or Optimizers.')
    
    if loss is not None and not isinstance(loss, (TorchModule, CustFuncLoader)):
        raise ValueError(f'The loss function is of type {type(loss)}, not TorchModule or CustFuncLoader.')
